from deepjudge_challenge.StreamingJsonParser import StreamingJsonParser
import json
...
def test_complete_object():
    ...
    parser.consume('} }')
    result = parser.get()
    assert(json.dumps(result) == '{"user": {"id": 1041, "name": "Alice", "profile": {"age": 29, "bio": "Cybersecurityenthusiast", "location": {"city": "Zurich", "zip": 8001}}}, "settings": {"theme": "dark", "notifications": {"email": true, "sms": false, "frequency": {"daily": 1, "weekly": 0}}}, "stats": {"logins": 145, "lastLogin": {"day": "Monday", "hour": 14}}}')